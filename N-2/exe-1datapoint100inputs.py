...
for sample in sample_list:
    ...
    for (i,std_w1) in zip(range(len(axis_std_w1)),axis_std_w1):
        ...
        for (j,std_w2) in zip(range(len(axis_std_w2)), axis_std_w2):
            ...
            optimizer = snn.init_train(std_w1,std_w2, lr=1e-4, **wparams)
            log_softmax_fn = nn.LogSoftmax(dim=1) # The log softmax function across output units
            loss_fn = nn.NLLLoss() # The negative log likelihood loss function

            for e in range(epoch):
                output, spks = snn.run_snn(x_data, device, dtype, **wparams)
                spike_count =torch.sum(output,1)
                mean_firing_rate = spike_count/(wparams['nb_steps']*wparams['time_step'])
